...
class GifMode(LEDmode):
    ...
    def __init__(self, matrix: RGBMatrix):
        self.matrix = matrix
        startup = Image.open(GifConstants.STARTUP)
        idle = Image.open(GifConstants.IDLE)

        try:
            self.startup_frames = startup.n_frames
            self.idle_frames = idle.n_frames
        except Exception:
            sys.exit("rovided image is not a gif")

        self.startupCanvases = compileGif(startup, matrix)
        self.idleCanvases = compileGif(idle, matrix)
        startup.close()
        idle.close()
